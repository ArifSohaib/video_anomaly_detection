...
def main():
    ...
    full_percentile = np.percentile(mse_full, 75)
    impact_percentile = np.percentile(mse_impact, 75);
    print("full mse 25 {}".format(full_percentile))
    print("impact mse 25 {}".format(impact_percentile))
    ...
    for idx, err in enumerate(mse_full):
        # if err > mse_impact.max():
        #     pred_impact_idx.append(idx)
        if err > full_percentile:
            pred_impact_idx.append(idx)
